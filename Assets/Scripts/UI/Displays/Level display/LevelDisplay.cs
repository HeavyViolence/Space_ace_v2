...
namespace SpaceAce.UI.Displays
{
    public sealed class LevelDisplay : UIDisplay
    {
        ...
        private string _gunSizeCode, _ammoTypeCode;
        ...
        public async UniTask DisplayPlayerShipViewAsync(IEntityView view, CancellationToken token = default)
        {
            ...
            await UpdatePlayerShipWeaponViewAsync(view.ShooterView);
            ...
            view.ShooterView.WeaponChanged += async (_, _) => await UpdatePlayerShipWeaponViewAsync(view.ShooterView);
            view.ShooterView.OutOfAmmo += PlayerShipOutOfAmmoEventHandler;
            view.ShooterView.AmmoChanged += (_, _) => UpdatePlayerShipAmmoView(view.ShooterView);
        ...
        private void UpdatePlayerShipAmmoView(IShooterView view)
        {
            _playerShipAmmoLabel.text = $"{_gunSizeCode}.{_ammoTypeCode}.{view.GetDamagePerSecond():n0}/{view.ActiveAmmo.Amount:n0}";
        }
        ...
        {
            Color heatColor = view.Overheat == true ? _config.GetHeatColor(1f) : _config.GetHeatColor(view.HeatNormalized);

            _playerShipHeatBar.style.width = Length.Percent(view.HeatPercentage);
            _playerShipHeatBar.style.backgroundColor = new(heatColor);
        }

        private void PlayerShipOutOfAmmoEventHandler(object sender, OutOfAmmoEventArgs e)
        {
            _playerShipAmmoLabel.text = $"{_gunSizeCode}.{e.Warning}";
        }
        ...
        public async UniTask DisplayTargetViewAsync(IEntityView view, CancellationToken token = default)
        {
            ...
            await UpdateTargetWeaponViewAsync(view.ShooterView);
            ...
            {
                view.ShooterView.AmmoChanged += async (_, _) => await UpdateTargetWeaponViewAsync(view.ShooterView);
                view.ShooterView.WeaponChanged += async (_, _) => await UpdateTargetWeaponViewAsync(view.ShooterView);
            }
        ...
        private async UniTask UpdateTargetWeaponViewAsync(IShooterView view)
        {
            if (view is null)
            {
                _targetAmmoLabel.text = await Localizer.GetLocalizedStringAsync("Guns", "No guns code");
            }
            else
            {
